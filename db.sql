...
insert into super_admin(username,password,lastLogin)values('super','super',now());
-------------------------shaurya end---------------------------
...
insert into Du_No(name,enrollment,Du_No)values("sunidhi",150167,"DU167");
insert into Du_No(name,enrollment,Du_No)values("semma",150168,"DU168");
insert into Du_No(name,enrollment,Du_No)values("monika",150169,"DU169");
insert into Du_No(name,enrollment,Du_No)values("anoushka",150170,"DU170");
insert into Du_No(name,enrollment,Du_No)values("surbhi",150171,"DU171");
...
DROP TABLE IF EXISTS `mydb`.`Student_Login` ;

CREATE TABLE IF NOT EXISTS `mydb`.`Student_Login` (
  `hostelId` INT NOT NULL,
  `userName` INT NULL,
  `password` VARCHAR(45) NULL,
  `lastLogin` TIMESTAMP(4) NULL,
  PRIMARY KEY (`hostelId`),
  CONSTRAINT `hostelId_login`
    FOREIGN KEY (`hostelId`)
    REFERENCES `mydb`.`Student` (`hostelId`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
insert into student_login(hostelId,username,password,lastLogin)values(1000,150169,'sma02*ST',now())
